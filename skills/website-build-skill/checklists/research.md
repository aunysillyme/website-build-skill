...
When a shell exists, save the exact `website-build-skill check-library <root>/site-work`
command, exit code and output with the gate receipt. A repository checkout can use
`node bin/website-build-skill.mjs check-library <site-work-dir>`. Without a shell,
Reviewer records each rule's result, claim IDs and evidence paths by hand:
