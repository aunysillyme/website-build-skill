...
Status: UNVERIFIED. Account controls, skill upload availability and exact menu labels need a live check.
Available manual route: attach the five bundles and paste the [shared instructions](chatgpt-project.md#instruction-block).
Planned, unavailable: download a release `website-build-skill.zip`; no release asset exists yet.
A future skill ZIP contains one complete `website-build-skill/` directory, including its manifest and relative resources.
Where an account exposes skill upload, upload that complete ZIP and confirm the actual loaded files.
A terminal-host copy does not prove ordinary desktop chat loaded anything.
Reference target: [Use skills](https://support.claude.com/en/articles/12512180-use-skills-in-claude).
