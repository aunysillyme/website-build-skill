...
```text
For website work, read .website-build-skill/SKILL.md. Use option 1, Teach my AI.
Keep site-work in this authorized project. Researcher works first; preserve all existing instructions.
```

Confirm the agent reads the pointer and core before calling it activated.
Check router discovery against the installed version of Cursor, Copilot, Jules or your chosen client.
A human can paste the pointer explicitly when automatic discovery is unavailable.
Install the files: `npx website-build-skill --solo --target agents --dir . --yes`.
